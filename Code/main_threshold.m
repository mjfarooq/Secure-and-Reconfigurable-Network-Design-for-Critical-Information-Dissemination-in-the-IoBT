...
ind = 0;
T1_V = 0.1:0.1:0.9;
for Tc = 0.1:0.1:0.9 % threat level
    ind = ind+1;
    alpha = 1 - delta;
%     T2 = T1;
...
%figure;
% pp1 = plot(T1_V,1000*x_opt(:,3),'-*r', 'Linewidth', 1.2); % Radius
% hold on
% pp2 = plot(T1_V,1000*x_opt(:,4),'--ob', 'Linewidth', 1.2); % Radius
% grid on
% xlabel('Network-wide information dissemination threshold, $T^{(c)}$','Interpreter','latex')
...
% figure;
pp3 = plot(T1_V,x_opt(:,1).*x_opt(:,2),'-*r', 'Linewidth', 1.2) % Intensity
hold on
pp4 = plot(T1_V,x_opt(:,2),'--ob', 'Linewidth', 1.2) % Intensity
grid on
xlabel('Network-wide information dissemination threshold, $T^{(c)}$','Interpreter','latex')
...
% figure
% plot(Tc_V,fval, '-xb' ,'Linewidth', 1.2) % Plot of the Cost function
% grid on
% xlabel('Network-wide information dissemination threshold, $T^{(c)}$','Interpreter','latex')
